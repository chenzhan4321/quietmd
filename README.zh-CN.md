...
`-w` 模式下，读到哪儿还会写在文档旁边的 `.quietmd-<名字>.json` 里 ——
那个目录要是在 Dropbox 或仓库里，换台机器就能接着读。它记的是「读到哪一节」而不是像素偏移：
改字号、换风格、在上面加几段，它依然指着同一处。静态打开的 HTML 写不了文件，
那种情况还是像原来一样存在 localStorage。
...
正文里手写的目录也能点：标题叫「目录 / Contents」的那一节里，凡是能对上某个标题的条目
都会变成链接 —— 列表和「每行一条」的段落都认。对不上的条目原样保留，不会把正文里的
普通句子变成链接。单独一行写 `[TOC]` 则展开成完整的带链接目录。
...
一本写着「你可以自己跑」的书，读者还是得把每段代码抄进终端。有本地服务的时候不必这样：

```console
$ quietmd chapter.md -w --run
```

python 和 shell 代码块会多出一个运行按钮，输出显示在下面，工作目录就是文档所在的目录。

边界是特意收紧的，毕竟这是在执行代码：只在 `-w` 下、必须显式加 `--run`（默认关）、
只跑上面那几种语言、有超时也有输出上限。静态导出的 HTML 里没有这套东西 ——
这个能力不会跟着文件被分享出去。

## 让引用能点开

`[论文 §4.1]` 这样的写法本来只是文字。在 front matter 里把出处说一次，全篇的引用就都变成链接：
...
URL 里写 `{sec}` 会把节号填进去 —— `[论文 §3.2.2]` 配上 `…/html/2606.07502v1#S{sec}`
就直接落到那一节。像 `[论文 Table 1]` 这种没有节号的，会把锚点整个去掉、链到文献本身，
而不是拼出一个跳不到任何地方的碎片。

项目里的路径不用配置：`` `outputs/sts_results.json` `` **只在文件真的存在时**才变成链接，
随手写下的路径不会变成死链。
...
每张图钉在它后面那段文字旁边：左边扫描件不动，右边转写往下滚，滚到下一页时图自动换成
下一张。不用再来回切窗口对着看。图被限制在一屏之内 —— 钉住的图要是比屏幕还高，
下半页就永远看不见了；点图可以看原尺寸。
...
**公式是整条比的，不是逐字符比。** 逐字符 diff 会把一条长公式撕成满屏红绿点，
而你真正想知道的是「哪些公式变了」。只差空白的两条公式**不算改动** ——
数学模式下空格被忽略，`\langle u, v\rangle` 和 `\langle u,  v \rangle` 排出来一模一样；
把 `u_i` 改成 `v_i` 才算。顶部同时给出总改动数和其中涉及公式的数目，
一眼看出这次改稿动了多少数学。
...
article、一本书、或者双倍行距的校对稿。

纸上是重新排的一次版，不是屏幕的灰度截图。版心由页边距定 —— 左右各 32mm，A4 上一行落在
36 个汉字 / 72 个西文字符 —— 于是正文居中、左右留白对称，而不是横跨整张纸。标题不会被甩
在页脚，段落不会只剩一行掉在另一页，表格跨页时表头跟着重复，图片不会被腰斩；屏幕上横着能
滚的东西（长代码行、宽公式、宽表格）在纸上会折行或缩小，而不是被无声裁掉。站外链接后面跟
上地址 —— 纸是点不动的。

整张表不依赖任何背景色，因为浏览器默认就不打印背景：代码块改用细边框，表格用学术三线表，
版本对比用 `−` 和 `+` 而不是红绿（红和绿在黑白激光机上是同一个灰）。页码交给浏览器打印对
话框里的「页眉和页脚」，下边距已经给它留好了位置。
...
**阅读** —— 侧边目录跟随滚动、章可折叠（读到哪章哪章自己展开）、阅读进度条、明暗跟随系统、超长公式横向滚动、
