...
**阅读** —— 侧边目录跟随滚动高亮、阅读进度条、明暗跟随系统（`t` 手动切换）、
超长公式横向滚动带渐隐提示、双击公式复制 LaTeX、打印/存 PDF 样式、刷新后回到原位置。

**工具栏**做成一条排字房的标尺，而不是网页 header。分组之间用发丝线隔开；两个 stepper
显示的是**真实的排版参数** —— 一行能排多少个字（`70ch`）、正文多大（`17px`），
而不是「窄/适中/宽」。行宽是真量出来的，不是查表：它拿当前字体量一个字符宽再除正文宽度，
所以换风格、换字号它都会跟着动 —— 因为那才是事实。界面默认英文，一个 `中文` 按钮切换（`l`），
和风格、宽度、字号、明暗、目录状态一样都会被记住。
...
**键盘** —— `j`/`k` 滚动 · `g`/`G` 首尾 · `t` 明暗 · `s` 风格 · `h` 目录 · `l` 语言 ·
